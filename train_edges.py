...
train_images = im.h5py_to_array(train_images[:2000], im_shape)
train_labels = train_file["joint_vel"][:2000].astype(np.float32)
a_train_images = train_images[:1000]
a_train_labels = train_labels[:1000]
a_dataset_size = a_train_images.shape[0]

b_train_images = im.canny_edges(train_images[1000:], rescale=True)
b_train_labels = train_labels[1000:]
...
b_test_images = im.canny_edges(a_test_images, rescale=True)
